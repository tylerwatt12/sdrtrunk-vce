...
Choose the format explicitly, select a UTF-8 CSV, and press **Preview import**. Files are limited to 8 MiB and
...
## VCE configuration CSV, version 1

Export the entire selected list using the Export tab. The importer requires this exact header, including order
and capitalization:

```csv
format_version,name,description,group,color,icon,matcher_type,protocol,value,minimum,maximum,text,tones,record_enabled,scan_lists,streaming_destinations,stream_as_talkgroup
```

All columns are required. `format_version` is `1`. A matching existing alias receives all the supplied configuration,
...
apostrophe. Version 1 imports remove that prefix from text fields. Preserve this encoding when editing a CSV:
...
appearance, and stream-as override remain intact. New aliases use Alias List Defaults unless the modal supplies
recording or assignment overrides. Type an exact configured name and press **Add** for each scan list or streaming
destination. Enable **Choose assignments** with no selected names to explicitly choose none.
