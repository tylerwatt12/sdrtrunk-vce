...
An accepted predecessor-database migration preserves supported configuration, including channels, Aliases, settings,
streaming definitions, and routes, but it does not carry API evidence forward. The Application Migrator creates a
safety backup, builds and validates a staged copy, and promotes it atomically. All previous statistics, detailed
Activity, identities, affiliations, site presence, observations, and counters are discarded, and the current
statistics and activity schema starts empty. API totals and history therefore begin with observations collected after
the migration; no dynamic evidence is reconstructed from configuration or older summaries.
