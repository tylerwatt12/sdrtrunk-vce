...
Statistics removes both tables through the same channel-owned lifecycle.
...
column. No secondary index is required:
...
Representative-volume tests verify those indexed searches and the fixed response limits.
