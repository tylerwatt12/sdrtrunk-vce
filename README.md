...
- The audio layer now models the distinction between a still-open call segment and the individual talk bursts inside it. `AudioSegment` carries explicit liveness and burst state, `P25P2AudioModule` can open a segment at `PTT`/`ESS` before the first committed audio frame, and burst boundaries are tracked independently of final segment closure. This is an architectural change aimed at making Phase 2 HANGTIME/inter-burst behavior explicit instead of forcing playback to infer everything from timing alone.
- The audio ownership model was also hardened. `AudioPlaybackManager.dispose()` now releases any segments it still owns instead of silently clearing those collections, and `AudioSegment` now warns on reference-count underflow or increment-after-dispose so lifecycle bugs fail loudly instead of quietly leaking or double-freeing.
