...
- Phase 2 audio tone handling now suppresses short AMBE tone artifacts before they become either emitted tone metadata or audible playback. Brief one- or two-frame misclassifications were showing up as spurious tones and chirps; those frames are now held out unless they persist long enough to look real.
- The local playback path had an audio drop bug: `AudioChannel` could dispose an incomplete segment after a short no-audio interval even though the decoder was still appending more audio to that same segment. Trace work showed that Phase 2 calls can legitimately resume on the same open segment after inter-burst gaps well over one second, so the old reclaim rule was simply wrong. The playback path now uses producer activity tracking and a much longer stale timeout for incomplete segments, while still retiring completed segments promptly and reclaiming truly abandoned live segments.
- The stereo playback scheduler also had an availability bug: a channel could still be treated as available if it was actively playing but its follow-on queue happened to be empty. `AudioChannel.isEmpty()` has been corrected and renamed to `isIdle()`, so playback assignment now only targets channels that are truly idle. This bug caused the left channel to be used almost exclusively, even when the right channel was available.
- `State.ENCRYPTED` could previously not transition directly back to `ACTIVE`, `CALL`, or `DATA`, which made encrypted-to-clear reuse of a Phase 2 traffic timeslot an internal state-machine dead end. That transition table has been corrected so reused timeslots can move cleanly back into active clear or data states.
- The audio layer now models the distinction between a still-open call segment and the individual talk bursts inside it. `AudioSegment` carries explicit liveness, lifecycle, and burst state; `P25P2AudioModule` can open a segment at `PTT`/`ESS` before the first committed audio frame; burst boundaries are tracked independently of final segment closure; and playback reclaim is now burst-aware, using different stale behavior for an unexpectedly stalled active burst versus a normal between-burst HANGTIME pause.
- Playback now reacts more directly to producer-side lifecycle changes. `AudioSegment` emits explicit segment and burst lifecycle transitions, `AudioPlaybackManager` uses those transitions to promote or retire changed segments sooner instead of waiting to rediscover them by polling, and `AudioChannel` now consumes a dedicated `AudioPlaybackBurst` view rather than treating the raw `AudioSegment` as both the call/session object and the playback-burst object at the same time.
- The audio ownership model was also hardened. `AudioPlaybackManager.dispose()` now releases any segments it still owns instead of silently clearing those collections, playback ownership/release paths are centralized instead of open-coded at every call site, and `AudioSegment` now warns on reference-count underflow or increment-after-dispose so lifecycle bugs fail loudly instead of quietly leaking or double-freeing.
