...
- A quick JFR pass showed that memory pressure was dominated by primitive-array churn in the polyphase channelizer, especially `float[]` allocations in `ComplexPolyphaseChannelizerM2`. Two easy fixes helped a lot: the channelizer now accumulates directly instead of materializing a temporary multiply buffer for every pass, and it reuses its non-escaping accumulator scratch buffer instead of allocating it per transform. The IFFT handoff path was also trimmed to stop copying batch lists unnecessarily. On my captures, those small changes reduced channelizer-driven `float[]` allocation by roughly 80%. The remaining bulk of allocation in that path is now inside JTransforms' FFT implementation rather than in local scratch buffers. I investigated power of two FFT sizing for that allocation, but it's not viable here: the channel count is constrained by the 25 kHz P25 channel grid, and rounding to a power of two breaks that alignment.
...
- Phase 2 audio/call handling had a few confirmed correctness bugs. Informational `MAC_3_IDLE` and `MAC_6_HANGTIME` PDUs were being treated as if a live call had ended, scramble parameters were not always reaching traffic channels early enough, brief one- or two-frame AMBE tone misclassifications could leak through as spurious tones/chirps, and `State.ENCRYPTED` could not transition directly back to `ACTIVE`, `CALL`, or `DATA` on a reused timeslot. Those paths are now handled correctly.
- The shared playback layer also had confirmed policy bugs. `AudioChannel` could reclaim a live incomplete segment after too short a quiet interval even though the decoder was still appending audio, `AudioChannel.isEmpty()` could treat an actively playing channel as available simply because its follow-on queue was empty, and completed segments could be dropped if they finished before any output became idle. Playback now uses truthful idle detection, keeps completed audio queued until it can actually be played, and uses producer/burst activity to distinguish a genuinely abandoned segment from a normal inter-burst pause.
- More broadly, the audio path has been incrementally cleaned up so playback is not forced to infer everything from mutable shared state and timing alone. `AudioSegment` now carries explicit lifecycle and burst state, `P25P2AudioModule` can open a segment at `PTT`/`ESS` before the first committed audio frame, playback reacts to segment/burst lifecycle transitions instead of waiting to rediscover them by polling, and `AudioChannel` consumes a dedicated `AudioPlaybackBurst` view rather than treating the raw `AudioSegment` as both the call/session and playback-burst object. As part of that shift, the old `100 ms` playback-manager polling loop has been demoted to a `1 s` watchdog; normal playback now advances on explicit events, and the periodic pass exists mainly to expose any missed edge rather than to carry the design. Playback ownership/release paths are also centralized and instrumented so refcount bugs fail loudly instead of quietly leaking or double-freeing.
