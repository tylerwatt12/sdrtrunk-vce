...
public class DecoderFactory
{
    ...
    public static List<Module> getPrimaryModules(Channel channel, AliasModel aliasModel,
                                                 ...
                                                 ChannelActivityModel channelActivityModel)
    {
        DecodeConfiguration decodeConfig = channel != null ? channel.getDecodeConfiguration() : null;

        if(decodeConfig == null || decodeConfig.getDecoderType() == null || !decodeConfig.getDecoderType().isActive())
        {
            throw new IllegalArgumentException("Cannot create modules for a retired or unsupported decoder");
        }
                                         ...
                                         ChannelActivityModel channelActivityModel)
    {
                ...
                shouldAddP25ControlChannelRotationMonitor(channel, sctmf))
        {
            ...
            modules.add(new ChannelRotationMonitor(activeStates,
                ...
                P25ControlChannelRotationPolicy.ACTIVE_STATE_LOSS_GRACE_MILLISECONDS, userPreferences));
        }
    }
