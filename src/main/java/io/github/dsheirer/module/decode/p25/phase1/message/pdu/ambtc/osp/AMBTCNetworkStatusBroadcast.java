...
public class AMBTCNetworkStatusBroadcast extends AMBTCMessage implements IFrequencyBandReceiver
{
    ...
    public ScrambleParameters getScrambleParameters()
    {
        ...
        {
            int wacn = ((Number)Objects.requireNonNull(getWacn().getValue(), "WACN missing")).intValue();
            int system = ((Number)Objects.requireNonNull(getSystem().getValue(), "System missing")).intValue();
            int nac = ((Number)Objects.requireNonNull(getNAC().getValue(), "NAC missing")).intValue();
