...
/** Selects the fastest correct 15-tap real half-band decimation filter for the current CPU. */
public class RealHalfBand15TapFilterCalibration extends AbstractRealHalfBandFilterCalibration
{
    public RealHalfBand15TapFilterCalibration()
    {
        super(CalibrationType.FILTER_HALF_BAND_REAL_15_TAP, 15, false);
    }
    ...
    protected IRealDecimationFilter createFilter(Implementation implementation, float[] coefficients)
    {
        return switch(implementation)
        {
            ...
            case VECTOR_SIMD_64 -> new VectorRealHalfBandDecimationFilter15Tap64Bit(coefficients);
            case VECTOR_SIMD_128 -> new VectorRealHalfBandDecimationFilter15Tap128Bit(coefficients);
            case VECTOR_SIMD_256 -> new VectorRealHalfBandDecimationFilter15Tap256Bit(coefficients);
            case VECTOR_SIMD_512 -> new VectorRealHalfBandDecimationFilter15Tap512Bit(coefficients);
            default -> throw new IllegalArgumentException("Unsupported 15-tap implementation: " + implementation);
        };
    }
}
