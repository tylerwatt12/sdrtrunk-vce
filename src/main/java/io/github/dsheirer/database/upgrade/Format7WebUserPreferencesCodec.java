...
/** Strict frozen codec for the version-2 preference document stored by database format 7. */
final class Format7WebUserPreferencesCodec
{
    ...
    private static final int DEFAULT_CONVERSATION_BURST_LIMIT = 4;
    ...
    private Format7WebUserPreferencesCodec()
    {
    }
    ...
    {
        Format6WebUserPreferencesCodec.validate(json);
        ...
        int selected = playback.path("selected_scan_list_ids").size();
        if(selected > MAXIMUM_SELECTED_SCAN_LISTS)
        {
            throw new SelectedScanListLimitException(selected, MAXIMUM_SELECTED_SCAN_LISTS);
        }
    ...
    private static Document decode(String json) throws IOException
    {
        ...
        {
            throw new IOException("Version-2 web user preferences are invalid", exception);
        }
    }
    ...
    private static void requireBounded(String json) throws IOException
    {
        ...
        {
            throw new IOException("Version-2 web user preferences are missing or exceed the storage bound");
        }
    }
                            ...
                            Map<String,TableLayout> tables)
    {
        private Document
        {
            if(version != VERSION)
            {
                throw new IllegalArgumentException("Unsupported version-2 web user preference version");
            }
    ...
    static final class SelectedScanListLimitException extends IOException
    {
        private final int mSelected;
        private final int mMaximum;

        private SelectedScanListLimitException(int selected, int maximum)
        {
            super("Version-1 preferences select " + selected + " scan lists; version 2 supports at most " + maximum);
            mSelected = selected;
            mMaximum = maximum;
        }
