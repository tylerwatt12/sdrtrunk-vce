...
/**
 * Java-standard PBKDF2-HMAC-SHA-256 password verifier for the single web administrator.
 */
public final class Pbkdf2PasswordHasher
{
    ...
    public WebAdminCredential createCredential(String username, char[] password, long authGeneration)
    {
        ...
        try
        {
            return new WebAdminCredential(WebAdminCredential.CURRENT_VERSION,
                ...
                Base64.getEncoder().encodeToString(derived), positiveNow(), authGeneration);
        }
        finally
        {
    ...
    {
        Objects.requireNonNull(credential, "Web administrator credential cannot be null");
    ...
    private static byte[] derive(char[] password, byte[] salt, int iterations)
    {
        ...
        {
            throw new IllegalStateException("Required web administrator password algorithm is unavailable", exception);
        }
        finally
        {
    ...
    private static void requireNewPassword(char[] password)
    {
        if(password == null || password.length < MINIMUM_PASSWORD_CHARACTERS ||
            password.length > MAXIMUM_PASSWORD_CHARACTERS)
        {
            throw new IllegalArgumentException("Web administrator password length is outside safe bounds");
        }
    }
