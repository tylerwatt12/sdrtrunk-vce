...
/**
 * Small, account-wide admission limit applied before expensive password verification.
 *
 * <p>This sliding window complements the per-source failure throttle.  It prevents a burst spread across many source
 * addresses from continuously feeding the single PBKDF2 worker before any one source has accumulated failures.  State
 * is in-memory, fixed-capacity, and never retained as login history.</p>
 */
final class AccountLoginAdmissionLimiter
{
    ...
    Decision tryAcquire()
    {
        ...
        try
        {
            ...
            {
                long retryAt = saturatingAdd(mAdmittedAtEpochMillis.getFirst(),
                    mConfiguration.window().toMillis());
