...
public class ThemeManager
{
    ...
    private final java.util.Map<java.awt.Component, java.awt.Font> mExplicitBaseFonts =
        java.util.Collections.synchronizedMap(new java.util.WeakHashMap<>());
    private final AtomicBoolean mJavaFxWindowListenerAttached = new AtomicBoolean();
    private final AtomicBoolean mJavaFxWindowListenerAttachPending = new AtomicBoolean();
    ...
    {
        if(mJavaFxWindowListenerAttached.get() ||
                !mJavaFxWindowListenerAttachPending.compareAndSet(false, true))
        {
            return;
        }
        ...
        Runnable attach = () -> {
            try
            {
                ...
                mJavaFxWindowListenerAttached.set(true);
            ...
            {
                mJavaFxWindowListenerAttachPending.set(false);
            }
        };
        ...
        else
        {
            ...
            {
                //FX toolkit is not started yet.  Clear the pending guard so the first explicit
                //Scene registration can attach the listener after JFXPanel starts the toolkit.
                mJavaFxWindowListenerAttachPending.set(false);
            }
        }
    }
    ...
    public void register(Scene scene)
    {
        ...
        //initialize() normally runs before the JavaFX toolkit exists.  The first registered Scene
        //is therefore the reliable point to retry the global Window listener used for dialogs.
        attachJavaFxWindowListener();
    ...
    private String buildAccentStylesheet(Theme theme)
    {
        try
        {
            ...
            return toDataStylesheetUrl(css.toString());
        }
        catch(Throwable t)
        {
    ...
    /**
     * Encodes CSS for a JavaFX data URI.  URLEncoder emits '+' for spaces, but JavaFX's data-URI
     * decoder treats '+' literally, so spaces must use their RFC 3986 percent form.
     */
    static String toDataStylesheetUrl(String css)
    {
        String encoded = java.net.URLEncoder.encode(css, java.nio.charset.StandardCharsets.UTF_8)
            .replace("+", "%20");
    ...
    private void applySwingLookAndFeel(Theme theme)
    {
        try
        {
            runOnSwingEventThreadAndWait(() -> applySwingLookAndFeelOnEdt(theme));
        }
    ...
    /**
     * Runs a Swing mutation synchronously on the event dispatch thread.
     */
    static void runOnSwingEventThreadAndWait(Runnable runnable)
    {
        ...
        catch(InterruptedException e)
        {
            ...
            throw new IllegalStateException("Interrupted while applying desktop appearance", e);
        }
        catch(java.lang.reflect.InvocationTargetException e)
        {
            throw new IllegalStateException("Unable to apply desktop appearance", e.getCause());
        }
    }

    private void applySwingLookAndFeelOnEdt(Theme theme)
    {
    ...
    private void refreshSwingComponent(java.awt.Component component)
    {
        Runnable refresh = () -> {
            SwingUtilities.updateComponentTreeUI(component);
            applyExplicitFontScale(component);
        };
    ...
    /**
     * FlatLaf scales look-and-feel fonts automatically.  A few radio controls intentionally use
     * explicit monospaced fonts, so retain their unscaled base font and apply the same user scale.
     */
    private void applyExplicitFontScale(java.awt.Component component)
    {
        ...
        if(font != null && !(font instanceof javax.swing.plaf.UIResource))
        {
            ...
            synchronized(mExplicitBaseFonts)
            {
                baseFont = mExplicitBaseFonts.computeIfAbsent(component, ignored -> font);
            }

            float scaledSize = (float)Math.max(1.0d, baseFont.getSize2D() * mCurrentGuiScale);
