...
public class NXDNConfigurationEditor extends ChannelConfigurationEditor
{
     ...
     */
    public NXDNConfigurationEditor(ConfigurationManager configurationManager, TunerManager tunerManager,
    ...
    private SegmentedButton getTransmissionModeButton()
    {
        if(mTransmissionModeButton == null)
        {
            ...
            ToggleButton tbTypeD = new ToggleButton(TransmissionMode.TYPE_D.getLabel());
            tbTypeD.setUserData(TransmissionMode.TYPE_D);
            mTransmissionModeButton.getButtons().add(tbTypeD);
    ...
    /**
     * Toggle switch for ignore data calls.
     */
    private ToggleSwitch getIgnoreDataCallsToggle()
    {
        if(mIgnoreDataCallsToggle == null)
        {
            mIgnoreDataCallsToggle = new ToggleSwitch("Data Calls");
            mIgnoreDataCallsToggle.setDisable(true);
            mIgnoreDataCallsToggle.selectedProperty().addListener((o, ov, nv) -> modifiedProperty().set(true));
        }

        return mIgnoreDataCallsToggle;
    }

    /**
     * Toggle switch for ignore encrypted calls.
     */
    private ToggleSwitch getIgnoreEncryptedCallsToggle()
    {
        if(mIgnoreEncryptedCallsToggle == null)
        {
            mIgnoreEncryptedCallsToggle = new ToggleSwitch("Encrypted Calls");
            mIgnoreEncryptedCallsToggle.setDisable(true);
            mIgnoreEncryptedCallsToggle.selectedProperty().addListener((o, ov, nv) -> modifiedProperty().set(true));
        }
    ...
    protected void setDecoderConfiguration(DecodeConfiguration config)
    {
        ...
        if(config instanceof DecodeConfigNXDN configNXDN)
        {
            ...
            getEncodingComboBox().getSelectionModel().select(configNXDN.getEncoding());
