...
package io.github.dsheirer.gui.configuration.alias.action;
 ...
 */
public class ScriptEditor extends ActionEditor<ScriptAction>
{
    private static final Logger mLog = LoggerFactory.getLogger(ScriptEditor.class);
    ...
    public ScriptEditor()
    {
        ...
        Label fileLabel = new Label("Script File");
        ...
        Label intervalLabel = new Label(("Run Script"));
    ...
    @Override
    public void setItem(ScriptAction scriptAction)
    {
        super.setItem(scriptAction);

        mComboBoxChangeListener.disable();
        mPeriodChangeListener.disable();

        getIntervalComboBox().setDisable(scriptAction == null);
        getFileChooser().setDisable(scriptAction == null);

        if(scriptAction != null)
        {
            RecurringAction.Interval interval = scriptAction.getInterval();
            getIntervalComboBox().getSelectionModel().select(interval);
            getPeriodFormatter().setValue(scriptAction.getPeriod());

            if(scriptAction.getScript() != null)
            {
                getFilePath().setText(scriptAction.getScript());
            }
            else
            {
    ...
    /**
     * Tests the currently loaded script action
     */
    private void test()
    {
        if(getItem() != null)
    ...
    private TextField getFilePath()
    {
        if(mFilePath == null)
        {
            ...
            mFilePath.textProperty().addListener((observable, oldValue, newValue) -> {
                ...
                {
                    getItem().setScript(newValue);
                    getTestButton().setDisable(false);
                }
                else
                {
                    getItem().setScript(null);
