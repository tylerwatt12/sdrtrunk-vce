...
public class ChannelEditor extends SplitPane implements IFilterProcessor, IAliasListRefreshListener
{
     ...
     * @param configurationManager containing configuration data and channel configurations
     */
    public ChannelEditor(ConfigurationManager configurationManager, TunerManager tunerManager, UserPreferences userPreferences)
    {
        mConfigurationManager = configurationManager;
        mConfigurationManager.addAliasListRefreshListener(this);
        mTunerManager = tunerManager;
        mUserPreferences = userPreferences;
        mUnknownConfigurationEditor = new UnknownConfigurationEditor(mConfigurationManager, mTunerManager,
        ...
        else
        {
            ...
            else
            {
                ...
                if(editorDecoderType == null || editorDecoderType != channelDecoderType)
                {
                    ...
                    if(editor == null)
                    {
                        editor = ChannelConfigurationEditorFactory.getEditor(channelDecoderType, mConfigurationManager,
    ...
    private TableView<Channel> getChannelTableView()
    {
        if(mChannelTableView == null)
        {
            ...
            mChannelFilteredList = new FilteredList<>(mConfigurationManager.getChannelModel().channelList(), mChannelListFilter);
                ...
                if(selected != null)
                {
                    ...
                    if(result.get() == ButtonType.YES)
                    {
                        if(selected.isProcessing())
                        {
                            try
                            {
                                mConfigurationManager.getChannelProcessingManager().stop(selected);
                            }
                            catch(Exception e)
                            {
                        ...
                        mConfigurationManager.getChannelModel().removeChannel(selected);
                    }
                }
            });
        }
