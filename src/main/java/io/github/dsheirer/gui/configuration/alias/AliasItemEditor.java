...
public class AliasItemEditor extends Editor<Alias>
{
    ...
    public void setItem(Alias alias)
    {
        ...
        if(alias != null)
        {
            ...
            getMonitorAudioToggleSwitch().setSelected(alias.isListen());
    ...
    boolean save(boolean reselectSavedAlias)
    {
        ...
        replacement.setListen(getMonitorAudioToggleSwitch().isSelected());
    ...
    private GridPane getTextFieldPane()
    {
        if(mTextFieldPane == null)
        {
            ...
            GridPane.setConstraints(colorLabel, 4, row);
            mTextFieldPane.getChildren().add(colorLabel);
            GridPane.setConstraints(getColorPicker(), 5, row);
    ...
    private Collection<String> getGroupSuggestions(AutoCompletionBinding.ISuggestionRequest request)
    {
        ...
        return mConfigurationManager.getAliasModel().getGroupNames().stream()
