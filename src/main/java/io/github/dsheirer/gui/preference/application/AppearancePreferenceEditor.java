...
public class AppearancePreferenceEditor extends HBox
{
    ...
    private Slider getGuiScaleSlider()
    {
        if(mGuiScaleSlider == null)
        {
            ...
            mGuiScaleSlider.setMajorTickUnit(0.5d);
            mGuiScaleSlider.setMinorTickCount(4);
            ...
            mGuiScaleSlider.valueProperty().addListener((obs, oldV, newV) -> {
                if(newV != null)
                {
                    ...
                    //Keyboard changes do not enter valueChanging=true and do not generate a
                    //mouse-release event, so persist them here.  Mouse interaction is committed
                    //by the listeners below after the user releases the slider.
                    if(!mGuiScaleSlider.isValueChanging() && !mGuiScaleSlider.isPressed())
                    {
                        persistGuiScale(newV.doubleValue());
                    }
                }
            });
            ...
            mGuiScaleSlider.valueChangingProperty().addListener((obs, wasChanging, changing) -> {
                ...
                {
                    persistGuiScale(mGuiScaleSlider.getValue());
                }
            });
    ...
    private void persistGuiScale(double scale)
    {
        if(Math.abs(mApplicationPreference.getGuiScale() - scale) > 1e-6)
        {
            mApplicationPreference.setGuiScale(scale);
        }
    }
